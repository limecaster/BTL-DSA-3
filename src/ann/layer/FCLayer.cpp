...
//     // Update m_unSample_Counter
//     m_unSample_Counter += DY.shape()[0];

//     const xt::xarray<double> &X = m_aCached_X;

//     // dW = dY outer X^T
//     double_tensor outer = outer_stack(DY, X);
//     m_aGrad_W = xt::mean(outer, {0});

//     // db = mean(dy) along axis=0 (batch axis)
//     if (m_bUse_Bias)
//     {
