...
void hashDemo3() {
    ...
    hashMap.remove(pkey, pValue, &deleteKey, &deleteKey);
    delete pkey;
    delete pValue;
    hashMap.println(&key2str, &value2str);
}
...
void hashDemo8()
{
    ...
    xmap<int, int> map2(map);
    map2.println();
    
    // Test assignment operator
    xmap<int, int> map3 = map;
    map3.println();
}
...
void hashDemo9()
{
    int keys[] = {2, 12, 42, 72, 3, 45, 76, 30};
    int values[] = {35, 67, 100, 23, 68, 68, 72, 45};
    xmap<int, int> *pMap = new xmap<int, int>(&xmap<int, int>::intKeyHash);
    for (int idx = 0; idx < 8; idx++) {
        pMap->put(keys[idx], values[idx]);
    }

    // Test copy constructor
    xmap<int, int> map(*pMap);
    map.println();

    // Test assignment operator
    xmap<int, int> map2 = *pMap;
    map2.println();
...
void hashDemo10()
{
    xmap<int*, int*> *pMap = new xmap<int*, int*>(
            &hashFunc,
            0.75,
            &valueEQ,
            &xmap<int*, int*>::freeValue,
    ...
    int values[] = {35, 67, 100, 23, 68, 68, 72, 45};
    for (int idx = 0; idx < 8; idx++) {
        pMap->put(new int(keys[idx]), new int(values[idx]));
    }
    pMap->println(&key2str, &value2str);

    // Test copy constructor
    xmap<int*, int*> map(*pMap);
    map.println(&key2str, &value2str);

    // Test assignment operator
    xmap<int*, int*> map2 = *pMap;
    map2.println(&key2str, &value2str);
...
void hashDemo11()
{
    ...
    int* pkey = new int(42);
    int* pValue = new int(100);
    map.remove(pkey, pValue, &deleteKey, &deleteKey);
    delete pkey;
    delete pValue;
    map.println(&key2str, &value2str);
    pMap->println(&key2str, &value2str);
